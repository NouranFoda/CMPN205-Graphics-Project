...
our::Texture2D* our::texture_utils::empty(GLenum format, glm::ivec2 size){
    ...
    //Bind the empty texture into the frame buffer and edit it then remove from the frame buffer and return it  
    texture->bind();  
    glTexImage2D(GL_TEXTURE_2D, 0, format, size.x, size.y, 0, format, GL_UNSIGNED_BYTE, NULL); //Don't load any images
...
our::Texture2D* our::texture_utils::loadImage(const std::string& filename, bool generate_mipmap) {
    ...
    //GL_TEXTURE_2D is the target,
    //0 is the mipmap level, 
    //GL_RGBA8 is the format of color components, 
    //size.x and size.y are the width and height of texture, 
    //0 is the border,
    //GL_RGBA is the format of the pixel data, 
    //GL_UNSIGNED_BYTE is the type of the pixel data, 
