...
class EntityTestState: public our::State {
    ...
    void onDraw(double deltaTime) override {
        ...
        glm::mat4 viewProjection = glm::perspective(glm::radians(45.0f), (float)size.x / (float)size.y, 0.1f, 100.0f) * camera->getLocalToWorldMatrix();
        ...
        for(auto& entity : world.getEntities()){
            ...
            meshRenderer->material->setUniform("u_viewProjection", viewProjection);
            meshRenderer->material->setUniform("u_model", entity->getLocalToWorldMatrix());
